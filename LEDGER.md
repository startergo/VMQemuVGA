...
Last updated: 2026-08-19 late (HOST-SIDE BLIT IMPLEMENTED, all three layers — kext 9c907d8 (0x600C BLIT + re-blit rule), Mesa 959f1d86038+9265b1acd54 (winsys flush_frontbuffer real, both readbacks gated under SHIM_HOST_PRESENT); artifacts built, DEPLOY+BOOT PENDING guest reachability (mDNS dead AND sl-ip timeout — recorded reachability≠state class); boot outcomes pre-registered in Mesa ledger entry 21. Prior: TSD RESTORATION LANDED AND VERIFIED cross-repo (Mesa 503d14cadde, dylib 19d3f756 on the fence-era boot): the object-table uniform-crossing family went to ZERO — the two-thread stomp was the carrier. Every transport-layer bug class of the day now closed with a verified fix: is_busy famine → fences; serialization → fences (25×); uniform crossing → pthread TSD. Frontier reduced to ONE item: the recorded page-context shader family (entry 18) vs the empty layer tree — frames still uniformly zero with the compositor's own program fixed. Full arc: Mesa ledger entries 20–21. Entries below)
