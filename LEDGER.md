...
Last updated: 2026-08-20 night (0x600C DELIVERY FALSIFIED AT PIXEL LEVEL — SPICE "gl draw" events fire (1328 re-issues of the probe rect) with ZERO pixels on screen (host capture of the extended display, image-checked); the GL-blit-into-2D-dst premise does not deliver — host writes never reach the guest-backed desktop surface. Same session: the wire-67/RGBA rejection hypothesis FALSIFIED by probe_fmt_renderable v3 (interleaved, counterbalanced: 1531x904 wire-67 CLEAR+readback PASS 3/3 exact, wire-1 PASS 3/3, both renderable per capset) — the transport is healthy; and the browser's front resource is census-proven EMPTY (0/1384024 color, 0/692012 depth) while layer surfaces hold 100% content — the compositor never renders into the presenting drawable. TWO open defects: (1) 0x600C pixels — candidate fix = kernel relay via proven paths (transfer_from_3d → kext memcpy flip/swizzle → transferToHost2D). LIMITATION, recorded up front: that is a readback RELOCATED, not eliminated — still two guest↔host crossings and ~5.5 MB/frame; what it buys is the copy moving off the main thread onto the kext workloop (the starvation fix). It is NOT the zero-readback host-side present the original design promised; the bandwidth question stays open; (2) empty front — Mesa-side, attacked first (a perfect relay of an empty framebuffer still shows nothing). Full arc: Mesa ledger Aug 20 evening. Entries below)
