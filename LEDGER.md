...
Last updated: 2026-08-20 night (0x600C DELIVERY FALSIFIED AT PIXEL LEVEL — SPICE "gl draw" events fire (1328 re-issues of the probe rect) with ZERO pixels on screen (host capture of the extended display, image-checked); the GL-blit-into-2D-dst premise does not deliver — host writes never reach the guest-backed desktop surface. Same session: the wire-67/RGBA rejection hypothesis FALSIFIED by probe_fmt_renderable v3 (interleaved, counterbalanced: 1531x904 wire-67 CLEAR+readback PASS 3/3 exact, wire-1 PASS 3/3, both renderable per capset) — the transport is healthy. THE EMPTY FRONT IS THEN RESOLVED AS PREF CONTAMINATION: the active profile carried layers.offmainthreadcomposition.enabled=false + layout.frame_rate=5 (flagged for removal, never removed — every Aug 20 run inherited them); clean profile → window front 1384024/1384024, depth 692012/692012. ONE open defect remains: 0x600C pixels — candidate fix = kernel relay (transfer_from_3d → kext memcpy flip/swizzle → transferToHost2D). LIMITATION, recorded up front: a readback RELOCATED, not eliminated — still two guest↔host crossings and ~5.5 MB/frame; it buys the copy moving off the main thread onto the kext workloop (the starvation fix). NOT the promised zero-readback host-side present; bandwidth question stays open. FRAMING, kept distinct: 0x600C blocks the host-present OPTIMISATION, not the browser — with SHIM_HOST_PRESENT unset the readback path still renders as before; "this path is unfinished" ≠ "nothing works". Relay build notes: dst resource 1 = wire 1 B8G8R8A8 (confirmed at BOTH creation sites — VMVirtIOFramebuffer.cpp:1190-1192 open(), setupDisplayResource createResource2D); swizzle = per-pixel R↔B byte swap; the kext memcpy's Y-flip is the ONLY flip (the shim's downstream CTM flip is bypassed in host-present mode — cgl_shim.mm:1288 early return, verified in source; double-flip impossible); log the dst format at first blit so a screen capture is interpretable either way (colour-swapped output must not read as delivery-success-with-a-bug). Full arc: Mesa ledger Aug 20 evening. Entries below)
