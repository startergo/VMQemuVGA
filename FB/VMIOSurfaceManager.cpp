...
uint32_t CLASS::getBytesPerPixel(VMIOSurfacePixelFormat format)
{
    switch (format) {
        ...
        case VM_IOSURFACE_PIXEL_FORMAT_BGRA32:  // 'BGRA' - 32-bit BGRA
        case VM_IOSURFACE_PIXEL_FORMAT_RGBA32:  // 'RGBA' - 32-bit RGBA
        case VM_IOSURFACE_PIXEL_FORMAT_ARGB32:  // 'ARGB' - 32-bit ARGB
        case VM_IOSURFACE_PIXEL_FORMAT_ABGR32:  // 'ABGR' - 32-bit ABGR
            return 4;
            
        // 24-bit formats (3 bytes per pixel)
        case 0x52474220: // 'RGB ' - 24-bit RGB
        case 0x42475220: // 'BGR ' - 24-bit BGR
            return 3;
            
        // 16-bit formats (2 bytes per pixel)
        case 0x52353635: // 'R565' - 16-bit RGB 565
        case 0x42353635: // 'B565' - 16-bit BGR 565
        case 0x52353535: // 'R555' - 16-bit RGB 555
        case 0x42353535: // 'B555' - 16-bit BGR 555
            return 2;
            
        // 8-bit formats (1 byte per pixel)
        case 0x4C303030: // 'L00' - 8-bit Luminance
            return 1;
            
        // 16-bit formats with alpha (2 bytes per pixel)
        case 0x4C413030: // 'LA00' - 8-bit Luminance + Alpha
            return 2;
            
        // YUV formats - more complex, return average
        case 0x79757634: // 'yuv4' - YUV 4:2:0 (1.5 bytes per pixel average)
        case 0x32315659: // 'YV12' - YUV 4:2:0 Planar
        case 0x49343230: // 'I420' - YUV 4:2:0 Planar
        case 0x56555949: // 'IYUV' - YUV 4:2:0 Planar
        case 0x32307679: // 'yv02' - YVU 4:2:0
            return 1; // Return 1 for simplicity, actual calculation is more complex
            
        case 0x79757632: // 'yuv2' - YUV 4:2:2 (2 bytes per pixel average)
        case 0x32767579: // 'yvu2' - YVU 4:2:2
        case 0x32595559: // 'YUY2' - YUV 4:2:2 Packed
        case 0x59565955: // 'YVYU' - YVU 4:2:2 Packed
        case 0x55595659: // 'UYVY' - UYV 4:2:2 Packed
            return 2;
            
        case 0x79757620: // 'yuv ' - YUV 4:4:4 (3 bytes per pixel)
            return 3;
            
        // Compressed formats - return 0 to indicate variable size
        case 0x44585431: // 'DXT1' - S3TC DXT1
        case 0x44585433: // 'DXT3' - S3TC DXT3  
        case 0x44585435: // 'DXT5' - S3TC DXT5
        case 0x45544331: // 'ETC1' - ETC1 compression
        case 0x45544332: // 'ETC2' - ETC2 compression
        case 0x50565254: // 'PVRT' - PowerVR texture compression
            return 0; // Variable/compressed size
            
        // Video formats - codec specific
...
uint32_t CLASS::getPlaneCount(VMIOSurfacePixelFormat format)
{
    switch (format) {
        ...
        // Packed YUV formats (single plane but multi-component)
        case 0x32595559: // 'YUY2' - YUV 4:2:2 Packed
        case 0x59565955: // 'YVYU' - YVU 4:2:2 Packed
        case 0x55595659: // 'UYVY' - UYV 4:2:2 Packed
            return 1;
            
        // Semi-planar YUV formats (2 planes: Y + UV)
        case 0x79757632: // 'yuv2' - YUV 4:2:2 (can be 2-plane)
        case 0x32767579: // 'yvu2' - YVU 4:2:2 (can be 2-plane)
            return 2;
            
        // Planar YUV formats (3 planes: Y + U + V)
        case 0x79757634: // 'yuv4' - YUV 4:2:0 (can be 3-plane)
        case 0x79757620: // 'yuv ' - YUV 4:4:4 (can be 3-plane)
        case 0x32315659: // 'YV12' - YUV 4:2:0 Planar (3 planes)
        case 0x49343230: // 'I420' - YUV 4:2:0 Planar (3 planes)
        case 0x56555949: // 'IYUV' - YUV 4:2:0 Planar (3 planes)
        case 0x32307679: // 'yv02' - YVU 4:2:0 (3 planes)
            return 3;
...
IOReturn CLASS::synchronizeSurface(uint32_t surface_id)
{
    ...
    // If hardware surfaces are supported, perform GPU synchronization
    if (m_supports_hardware_surfaces && m_gpu_device) {
        // In a real implementation, we would:
        // 1. Flush any pending GPU operations on this surface
        // 2. Ensure coherency between CPU and GPU memory
        // 3. Wait for any outstanding GPU operations to complete
        
        // For now, we simulate the synchronization process
        IOLog("VMIOSurfaceManager: Synchronizing surface %u with GPU\n", surface_id);
        
        // Simulate sync delay (in a real implementation, this would be hardware-dependent)
        IOSleep(1); // 1ms delay to simulate GPU sync
...
{
    if (surface_id == 0 || !gpu_resource_id) {
        return kIOReturnBadArgument;
    }
    
    // Check if hardware surfaces are supported
    if (!m_supports_hardware_surfaces || !m_gpu_device) {
        IOLog("VMIOSurfaceManager: Hardware surfaces not supported for GPU resource creation\n");
        return kIOReturnUnsupported;
    }
    ...
    // Generate unique GPU resource ID
    static uint32_t next_gpu_resource_id = 1000;
    *gpu_resource_id = next_gpu_resource_id++;
    
    // Initialize GPU resources collection if needed
    if (!m_gpu_resources) {
        m_gpu_resources = OSArray::withCapacity(64);
        if (!m_gpu_resources) {
            IOLockUnlock(m_surface_lock);
            return kIOReturnNoMemory;
        }
    ...
    // Create GPU resource descriptor
    struct GPUResource {
        uint32_t resource_id;
        uint32_t surface_id;
        uint32_t resource_type; // 0 = texture, 1 = buffer, 2 = renderbuffer
        uint32_t width;
        uint32_t height;
        uint32_t pixel_format;
        uint64_t creation_time;
        ...
        bool active;
        bool coherent; // CPU-GPU memory coherent
    };
    
    GPUResource* gpu_resource = (GPUResource*)IOMalloc(sizeof(GPUResource));
    if (!gpu_resource) {
        IOLockUnlock(m_surface_lock);
        return kIOReturnNoMemory;
    }
    
    gpu_resource->resource_id = *gpu_resource_id;
    gpu_resource->surface_id = surface_id;
    gpu_resource->resource_type = 0; // Default to texture
    gpu_resource->width = surface->width;
    gpu_resource->height = surface->height;
    gpu_resource->pixel_format = surface->descriptor.pixel_format;
    gpu_resource->creation_time = mach_absolute_time();
    gpu_resource->last_sync_time = gpu_resource->creation_time;
    gpu_resource->active = true;
    gpu_resource->coherent = true; // Assume coherent memory for simplicity
    
    // Store GPU resource info
    OSData* resource_data = OSData::withBytes(gpu_resource, sizeof(GPUResource));
    if (!resource_data) {
        IOFree(gpu_resource, sizeof(GPUResource));
        IOLockUnlock(m_surface_lock);
        return kIOReturnNoMemory;
    }
    
    m_gpu_resources->setObject(resource_data);
    resource_data->release();
    
    // Update surface access time
    surface->last_access_time = mach_absolute_time();
    
    IOLog("VMIOSurfaceManager: Created GPU resource %u for surface %u (%ux%u, format: 0x%08X)\n",
          *gpu_resource_id, surface_id, surface->width, surface->height, surface->descriptor.pixel_format);
...
IOReturn CLASS::destroyGPUResource(uint32_t surface_id)
{
    ...
    // Find and remove GPU resources associated with this surface
    if (m_gpu_resources) {
        struct GPUResource {
            uint32_t resource_id;
            uint32_t surface_id;
        ...
        for (int i = (int)m_gpu_resources->getCount() - 1; i >= 0; i--) {
            ...
            if (gpu_resource->surface_id == surface_id) {
                ...
                gpu_resource->active = false;
                
                IOLog("VMIOSurfaceManager: Destroyed GPU resource %u for surface %u\n",
                      gpu_resource->resource_id, surface_id);
                
                // Remove from collection
                m_gpu_resources->removeObject(i);
                resource_found = true;
            }
        }
    }
    
    // Also clean up texture bindings
    if (m_texture_bindings) {
        struct TextureBinding {
            uint32_t surface_id;
            uint32_t texture_id;
        ...
        for (int i = (int)m_texture_bindings->getCount() - 1; i >= 0; i--) {
            OSData* binding_data = OSDynamicCast(OSData, m_texture_bindings->getObject(i));
            if (!binding_data) continue;
            
            TextureBinding* binding = (TextureBinding*)binding_data->getBytesNoCopy();
            if (!binding || !binding->active) continue;
            
            if (binding->surface_id == surface_id) {
                binding->active = false;
                m_texture_bindings->removeObject(i);
            }
        }
    }
    ...
    if (m_gpu_textures) {
        ...
        for (int i = (int)m_gpu_textures->getCount() - 1; i >= 0; i--) {
            OSData* texture_data = OSDynamicCast(OSData, m_gpu_textures->getObject(i));
            if (!texture_data) continue;
            ...
            if (gpu_texture->surface_id == surface_id) {
                gpu_texture->active = false;
                m_gpu_textures->removeObject(i);
            }
        }
    }
...
{
    // Standard pixel formats supported by IOSurfaces
    switch (format) {
        // RGB formats
        case 0x42475241: // 'BGRA' - 32-bit BGRA
        case 0x52474241: // 'RGBA' - 32-bit RGBA
        case 0x41524742: // 'ARGB' - 32-bit ARGB
        case 0x41424752: // 'ABGR' - 32-bit ABGR
        case 0x52474220: // 'RGB ' - 24-bit RGB
        case 0x42475220: // 'BGR ' - 24-bit BGR
        case 0x52353635: // 'R565' - 16-bit RGB 565
        case 0x42353635: // 'B565' - 16-bit BGR 565
